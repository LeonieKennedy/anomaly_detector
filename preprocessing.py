...
def adaptive_thresholding(image_file):
    ...
    new_img = cv2.adaptiveThreshold(img, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 333, 2)  # works better
...
def pre_process_image(image_path, thresholding, skew, noise):
    # Skew correction
    if skew is True:
        new_img = skew_correction(image_path)
        cv2.imwrite(image_path, new_img)

    # Adaptive thresholding
    elif thresholding is True:
        new_img = adaptive_thresholding(image_path)
        cv2.imwrite(image_path, new_img)
        new_img = cv2.imread(image_path)

    # Noise removal
    elif noise is True:
        new_img = noise_removal(image_path)
...
new_img = pre_process_image("fight.jpg", True, False,False)
